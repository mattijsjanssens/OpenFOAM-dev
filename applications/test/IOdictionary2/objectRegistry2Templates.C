...
template<class Type>
Foam::wordList Foam::objectRegistry2::names(const wordRe& name) const
{
    wordList objectNames(size());
...
template<class Type>
Foam::wordList Foam::objectRegistry2::names(const wordReList& patterns) const
{
...
template<class Type>
Foam::HashTable<const Type*> Foam::objectRegistry2::lookupClass
(
    const bool strict
) const
{
...
template<class Type>
Foam::HashTable<Type*> Foam::objectRegistry2::lookupClass
(
    const bool strict
)
{
    ...
    forAllIter(HashTable<regIOobject2*>, *this, iter)
    {
        if
        (
            (strict && isType<Type>(*iter()))
...
template<class Type>
bool Foam::objectRegistry2::foundObject(const word& name) const
{
...
template<class Type>
const Type& Foam::objectRegistry2::lookupObject(const word& name) const
{
    const_iterator iter = find(name);
    ...
    if (iter != end())
    {
        ...
        FatalErrorInFunction
            << nl
            << "    lookup of " << name << " from objectRegistry2 "
            << this->name()
    ...
    else
    {
        ...
        FatalErrorInFunction
            ...
            << " " << name << " from objectRegistry2 " << this->name()
