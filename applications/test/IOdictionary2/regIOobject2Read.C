...
Foam::Istream& Foam::regIOobject2::readStream()
{
    if (IFstream::debug)
    {
        InfoInFunction
...
Foam::Istream& Foam::regIOobject2::readStream(const word& expectName)
{
    if (IFstream::debug)
    {
        InfoInFunction
    ...
    if (!isPtr_)
    {
        ...
        // Check the className of the regIOobject2
...
bool Foam::regIOobject2::readData(Istream&)
{
    return false;
}


bool Foam::regIOobject2::read()
{
    // Note: cannot do anything in readStream itself since this is used by
    // e.g. GeometricField.

    bool masterOnly =
        regIOobject2::fileModificationChecking == timeStampMaster
     || regIOobject2::fileModificationChecking == inotifyMaster;
    ...
    if (Pstream::master() || !masterOnly)
    {
        if (IFstream::debug)
        {
            Pout<< "regIOobject2::read() : "
                << "reading object " << name()
                << " from file " << endl;
        }

        // Set flag for e.g. codeStream
        bool oldFlag = regIOobject2::masterOnlyReading;
        regIOobject2::masterOnlyReading = masterOnly;

        // Read file
        ok = readData(readStream(type()));
        close();

        regIOobject2::masterOnlyReading = oldFlag;
    }
...
bool Foam::regIOobject2::modified() const
{
    if (watchIndex_ != -1)
    {
...
bool Foam::regIOobject2::readIfModified()
{
    if (watchIndex_ != -1)
    {
        if (modified())
